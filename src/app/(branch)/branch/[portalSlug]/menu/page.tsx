...
export default function BranchPortalMenuPage() {
  const ctx = useBranchPortal();
  const { data: menu, isLoading } = useManagementMenu(ctx.branchId);
  const toggleMutation = useToggleAvailability(ctx.branchId);
  const deleteMutation = useDeleteItem(ctx.branchId);
  const deleteCategoryMutation = useDeleteCategory(ctx.branchId);
