...
interface OrderDetailPageProps {
  params: Promise<{ portalSlug: string; orderId: string }>;
}

export default function BranchPortalOrderDetailPage({
  params,
  ...
  const statusMutation = useUpdateOrderStatus(ctx.branchId);
  const confirmPaymentMutation = useConfirmPayment(ctx.branchId);
  const rejectPaymentMutation = useRejectPayment(ctx.branchId);

  const ordersHref = appRoutes.branchPortal.orders(ctx.portalSlug);
